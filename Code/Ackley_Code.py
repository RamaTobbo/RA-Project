...
# objective function
def ackley(position):
 return -20.0 * exp(-0.2 * sqrt(0.5 * (position[0]**2 + position[1]**2))) - exp(0.5 * (cos(2 * pi * position[0]) + cos(2 * pi * position[1]))) + e + 20
...
def gwo(fitness, max_iter, n, dim, minx, maxx):
    ...
    while Iter < max_iter:
        ...
        for i in range(n):
          tocsv.append([Iter,population[i].id,population[i].position[0]*60+300,population[i].position[1]*60+300])
...
pd.DataFrame(tocsv).to_csv("wolf_ackley.csv",header=False,index=False)


# In[5]:


pd.DataFrame(fit).to_csv("fitness_wolf_ackley.csv",header=False,index=False)
...
def abc_algorithm(obj_function, lb, ub, colony_size=10, max_iter=10 ,num_trials=5):
    """
    ...
    for iter in range(max_iter):
        ...
        for i in range(colony_size):
            ...
            if new_fitness < fitness[i]:
                ...
                alltrials[i] -= 1
        for i in range(colony_size):
            tocsv.append([iter,0,i,colony[i,0]*60+300,colony[i,1]*60+300])
            fit.append([iter,0,i,colony[i]])
        ...
        while i < colony_size:
            ...
            i += 1
        for i in range(colony_size):
            tocsv.append([iter,1,i,colony[i,0]*60+300,colony[i,1]*60+300])
        ...
        for i in range(colony_size):
            if alltrials[i] <= 0:
                ...
                alltrials[i] = trials
        for i in range(colony_size):
            tocsv.append([iter,2,i,colony[i,0]*60+300,colony[i,1]*60+300])
...
print(abc_algorithm(ackley,[-5,-5],[5,5]))


# In[9]:


pd.DataFrame(tocsv).to_csv("bee_ackley.csv",header=False,index=False)


# In[10]:


pd.DataFrame(fit).to_csv("fitness_bee_ackley.csv",header=False,index=False)
...
def Fun(D, sol):
    return -20.0 * exp(-0.2 * sqrt(0.5 * (sol[0]**2 + sol[1]**2))) - exp(0.5 * (cos(2 * pi * sol[0]) + cos(2 * pi * sol[1]))) + e + 20
            ...
            for i in range(self.NP):
                ...
                if Fnew <= self.f_min:
                    for j in range(self.D):
                        self.best[j] = S[i][j]
                    self.f_min = Fnew
                tocsv.append([t,i,self.Sol[i][0]*60+300,self.Sol[i][1]*60+300])
...
Algorithm = BatAlgorithm(2, 15, 20, 5, 0.5, 0.0, 2.0, -5.0, 5.0, Fun)
Algorithm.move_bat()
...
pd.DataFrame(tocsv).to_csv("bat_ackley.csv",header=False,index=False)


# In[18]:


pd.DataFrame(fit).to_csv("fitness_bat_ackley.csv",header=False,index=False)
    ...
    def search(self):
        ...
        for i in range(self.iterations_number):
            ...
            for j in range(0,len(self.cluster)):
                tocsv.append([i,0,j,self.cluster[j].current_position[0]*60+300,
                              self.cluster[j].current_position[1]*60+300,self.cluster[j].weight])
            ...
            for j in range(0,len(self.cluster)):
                tocsv.append([i,1,j,self.cluster[j].current_position[0]*60+300,
                              self.cluster[j].current_position[1]*60+300,self.cluster[j].weight])
            ...
            for j in range(0,len(self.cluster)):
                tocsv.append([i,2,j,self.cluster[j].current_position[0]*60+300,
                              self.cluster[j].current_position[1]*60+300,self.cluster[j].weight])
...
class Rosenbrocks(AFunction):
    ...
    def calculate_fitness(self, x):
        ...
        return sum_
class Ackley(AFunction):  
    def __init__(self):
        AFunction.upper_bound = 5
        AFunction.lower_bound = -5
...
import pandas as pd
pd.DataFrame(tocsv).to_csv("fish_ackley.csv",header=False,index=False)


# In[27]:


pd.DataFrame(fit).to_csv("fish_ackley_fitness.csv",header=False,index=False)
